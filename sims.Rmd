...
This file tries to simulate multi-level data for a range of factorial designs.
...
the faux::crossing() function is useful to set up the structure
...
lm1_power <- lmx1_params %>% 
  ...
  mutate(check = ifelse(p.value < 0.5, 1, 0)) %>% 
...
lm1_plot_params <- lmx1_params %>%
  filter(term == "grouptreatment") %>%
  mutate(below_05 = if_else(p.value < 0.5, "yes", "no"), 
...
some code to walk you through it.)
